...
void append_blanks_under_heads() {
    for (size_t a = 0; a < tapes.size(); ++a)
        if (heads[a] >= tapes[a].size()) tapes[a].emplace_back(BLANK);
}
...
void execute_step(const TuringMachine &tm) {
    ...
    if (tm.transitions.find(cur) == tm.transitions.end()) {
        if (verbose) cerr << "No transition from this configuration\n";
        halt(false);
    }
    ...
    for (size_t a = 0; a < tapes.size(); ++a) {
        ...
        if (dir == HEAD_LEFT && !heads[a]) {
            if (verbose)
                cerr << "Head " << a + 1
                     << " falls off the tape in the next transition\n";
            halt(false);
        }
...
void print_configuration() {
    ...
    for (size_t a = 0; a < tapes.size(); ++a) {
        ...
        for (size_t b = 0; b < tapes[a].size(); ++b) {
            if (b == heads[a]) before_head = oss.str().length();
            oss << tapes[a][b];
            if (b == heads[a]) after_head = oss.str().length();
        }
        cerr << oss.str() << "\n";
        for (size_t b = 0; b < before_head; ++b) cerr << " ";
        for (size_t b = before_head; b < after_head; ++b) cerr << "^";
        cerr << "\n";
    }
    cerr << "#####################################\n";
}
